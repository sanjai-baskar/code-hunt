...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen" style={{ background: '#0B0C10' }}>
      {/* Nav */}
      <nav className="sticky top-0 z-50" style={{ background: 'rgba(11,12,16,0.9)', backdropFilter: 'blur(12px)', borderBottom: '1px solid rgba(69,162,158,0.2)' }}>
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="text-2xl">🎯</span>
            <span className="text-xl font-bold text-white">Code<span style={{ color: '#45A29E' }}>Hunt</span></span>
            <span className="ml-2 text-xs px-2.5 py-1 rounded-full font-medium"
              style={{ background: 'rgba(69,162,158,0.15)', color: '#45A29E', border: '1px solid rgba(69,162,158,0.3)' }}>
              Admin
            </span>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="mb-8">
          ...
          <p style={{ color: 'rgba(226,232,240,0.5)' }}>Manage problems and monitor student activity.</p>
        </div>
          ...
          {[
            ...
            <div key={s.label} className="glass p-5 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm" style={{ color: 'rgba(226,232,240,0.5)' }}>{s.label}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex gap-2 mb-6">
          {['problems', 'students'].map((tab) => (
            <button key={tab} id={`tab-${tab}`} onClick={() => setActiveTab(tab)}
              ...
              style={{
                background: activeTab === tab ? 'rgba(69,162,158,0.2)' : 'rgba(255,255,255,0.05)',
                color: activeTab === tab ? '#45A29E' : 'rgba(226,232,240,0.6)',
                border: activeTab === tab ? '1px solid rgba(69,162,158,0.4)' : '1px solid rgba(255,255,255,0.08)',
              }}>
          ...
          <div>
            ...
            <div className="bg-[var(--bg-card)]">
              {loading ? (
                ...
                <>
                  {problems.map((p) => (
                    <div key={p.id} className="problem-row justify-between">
                      <div>
                        <p className="text-[var(--text-main)] font-medium mb-1">{p.title}</p>
                        <span className={`badge-${p.difficulty.toLowerCase()}`}>
                          {p.difficulty}
                        </span>
                      </div>
                      <div className="flex gap-4">
                        <button onClick={() => { setEditingProblem(p); setShowForm(true); }} className="text-[var(--text-muted)] hover:text-[var(--text-main)] text-sm">Edit</button>
                        <button onClick={() => deleteProblem(p.id)} className="text-[#ef4743] hover:text-red-400 text-sm font-medium">Delete</button>
                      </div>
                    </div>
                  ))}
                  {problems.length === 0 && (
                    <div className="p-8 text-center text-[var(--text-muted)]">No problems created yet.</div>
                  )}
                </>
              )}
            </div>
        ...
        {activeTab === 'students' && (
          <div>
            <h2 className="text-lg font-semibold text-[var(--text-main)] mb-4">Registered Students</h2>
                ...
                <div key={s.id} className="lc-card p-5 flex items-center justify-between">
                  <div>
                    ...
                    <div className="flex gap-4 mt-1">
                      ...
                      <span className="text-xs text-[var(--text-muted)]">{s._count?.distractionLogs || 0} distraction events</span>
                    </div>
                  </div>
                  <button onClick={() => setSelectedStudent(s)} className="bg-[var(--bg-dark)] border border-[var(--border-main)] text-[var(--text-main)] text-sm px-4 py-2 rounded hover:bg-[var(--border-main)]">
                    View Logs
                  </button>
                </div>
              ))}
